...
Slack 通知を使う場合は、`.env` に `SLACK_WEBHOOK_URL` を設定します。取得成功後に、対象期間・記事数・合計値・前週比・記事別スタッツ・CSV 保存先を通知します。記事別スタッツは Slack メッセージ上で確認できるよう、閲覧数順に最大20件まで表示し、各記事にも前週比を表示します。
